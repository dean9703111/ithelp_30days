...
4. 先透過閱讀套件的文件來第一步確認是否有你想要的功能
    * 通常在npm網頁裡面就會有套件的說明，但部分套件因為使用上相對複雜，詳細的使用方法會放在Homepage的連結，點擊後會導向更詳細的說明文件(也可能是套件官網)
...
2. **練習控制套件版本**
    1. 打開專案 **package.json** ，調整套件版本
    2. 在專案資料夾的終端機(Terminal)執行指令 **yarn** 安裝套件
    3. 在 **yarn.lock** 檔案觀察下載的套件版本是否正確
