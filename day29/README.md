...
* 完成爬蟲後回傳兩個參數：`result_array`、`error_title_array`，供後續資料串連
...
* 取得爬蟲開始、結束時間後我們用 `spendTime` 這個函式提高總費時易讀性
...
🚀 執行程式
----
1. 為了確認我們爬蟲摘要能紀錄無法爬蟲的頁面，`我在 ig.json、fb.json 裡面各自新增了兩個無效的粉專網址`
