...
### 1.2 練習將範例搬到專案：`getSheets`
在了解官方範例後，我們就在專案撰寫一個 `取得spreadsheetId下sheets資訊` 的函式吧
```js
...
2. 看[Add a sheet](https://developers.google.com/sheets/api/samples/sheet#add_a_sheet)的描述
    * 第一段就提供一個連結，並說`這方法能新增sheet到spreadsheet`
...
3. 接著來確認[Method: spreadsheets.batchUpdate](https://developers.google.com/sheets/api/reference/rest/v4/spreadsheets/batchUpdate)是否符合需求
    ...
    2. 按下 **EXCUTE** 按鈕後，確認自己的Google Sheets`真的成功新增一個title為test1的Sheet`
...
### 3.2 練習將範例搬到專案：`addSheets`
在了解官方範例後，我們就在專案撰寫一個 `用提供的title建立一個新Sheet，完成後回傳sheetId` 的函式吧
```js
...
### 4.2 將取得auth(認證)的步驟改寫為函式：`getAuth`
考慮到`每個Google Sheets的api全部都需要通過憑證取得授權才能操作，所以我把這個步驟獨立成一個函式`，由於取得授權這塊採用callback的函式，所以我們要用Promise的方式來處理。
```js
function getAuth () {
